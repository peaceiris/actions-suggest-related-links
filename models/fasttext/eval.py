...
parser.add_argument(
    "-train", "--train_document", type=str, required=True, help="path to train document"
)
parser.add_argument(
    "-test", "--test_document", type=str, required=True, help="path to test document"
)
...
for train in trains:
    sent1_emb = np.mean(
        [fasttext_model[x] for word in train.split()[1:] for x in word.split()], axis=0
    )
